    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 transition-all hover:shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex items-center gap-3 p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {t('gusts')}: {windGustKmh} km/h ({wind_gust_ms.toFixed(1)} m/s)
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center gap-1">
              {t('direction')}: {getWindDirectionIcon(wind_direction_cardinal)} {wind_direction_cardinal.toUpperCase()}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 p-4 bg-cyan-50 dark:bg-cyan-900/20 rounded-lg">
          ...
          <div>
            <p className="text-sm text-gray-600 dark:text-gray-400">{t('rain')}</p>
            <p className="text-xl font-bold text-gray-800 dark:text-white">
              {rain_day_mm} mm ({t('today')})
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {t('total')}: {rain_total_mm} mm
            </p>
          </div>
        </div>
      </div>
